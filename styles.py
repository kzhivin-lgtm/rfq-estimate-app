...
/* =============================================================================
   STYLE_TOC_UPLOAD_PROCESSING_V1_8_3

   PURPOSE OF THIS COMMENT BLOCK
   ---------------------------------------------------------------------------
   This file became risky because Streamlit components are not normal hand-written
   HTML. Many visible elements are generated by Streamlit and Emotion. Some parts
   are stable enough to style by data-testid, while other parts receive generated
   classes such as `st-emotion-cache-...`.

   This block is a human-readable map for the first upload screen and the
   processing screen only. It is intentionally verbose so that future edits do
   not accidentally destroy the upload rectangle behavior again.

   CURRENT SCOPE
   ---------------------------------------------------------------------------
   Covered here:
   1. First upload screen
   2. File uploader / upload rectangle / dropzone
   3. Upload dragover state
   4. Hero typography on the upload screen
   5. Processing screen visual layer, if edited later
   ...
   - proposal screen
   - PDF export
   - data model / Supabase logic
   - detection prompt logic

   GLOBAL DESIGN TOKENS USED BY THE CURRENT PRODUCT VISUAL SYSTEM
   ---------------------------------------------------------------------------
   Page background:
   - #F1EFEF
   ...
   ---------------------------------------------------------------------------
   Streamlit must stay in LIGHT theme.

   Do not re-enable automatic dark-mode overrides here.
   Do not rely on browser prefers-color-scheme for this app.
   Dark surfaces should be custom CSS blocks, not native Streamlit dark theme.

   Why:
   - native Streamlit dark mode changed uploader internals visually;
   - upload rectangle states became inconsistent;
   - dragover/hover styling became harder to control;
   - the first upload screen needs stable, controlled branding.

   ---------------------------------------------------------------------------
   TABLE OF CONTENTS FOR FIRST SCREEN / PROCESSING SCREEN CSS
   ---------------------------------------------------------------------------

   01. Base app theme
       Controls global background, typography defaults, surfaces and app-wide
       Streamlit visual reset.

   02. Upload screen hero
       The logo and headline on the first screen:
       - costerly.ai logo
       - Archivo Black hero text
   ...
   03. Hero font loading
       Archivo Black is loaded from Google Fonts.
       The hero must use real Archivo Black, not browser fake-bold.
       ...
       Do not change this to font-weight: 900. Archivo Black is a single-weight
       display font. Using 900 can create fake-bold rendering and visually
       change the letterforms.

   04. Upload rectangle / Streamlit file uploader
       Streamlit renders the uploader with generated markup. The important
       stable selectors are:

       div[data-testid="stFileUploader"]
       section[data-testid="stFileUploaderDropzone"]

       These selectors affect the visible upload rectangle.

       Button-only selectors are not enough. A selector like:

       section[data-testid="stFileUploader"] button

       changes only the internal browse button, not the full dropzone rectangle.

   05. Upload normal state
       Normal state means no file is dragged over the dropzone.
   ...
   06. Upload hover state
       Hover state means mouse is over the upload rectangle, but no file is being
       dragged.

       Hover must not be confused with dragover.

       Do not show the plus sign on simple hover.
       The plus sign is reserved for actual dragover only.

   07. Upload dragover state
       Dragover means a file is being dragged over the upload rectangle.

       This is the fragile part.

       Streamlit adds a generated Emotion class to the dropzone during dragover.
       In one working version this class was:

       st-emotion-cache-1n7lb4w

       The final restore block below uses this generated class intentionally.

       If Streamlit changes this class in a future version, the block will stop
       working. That is expected. In that case, do not rewrite the whole uploader.
       Only replace the generated class in the dedicated block below after
       checking the DOM in browser DevTools.

   08. Native Streamlit dragover text
       During dragover, Streamlit may inject or reveal native text/layers.
       This can bring back the turquoise/default visual state.

       The final restore block below hides that native span text only when the
       confirmed dragover class is present.

   09. Dragover inner visual layer
       Streamlit may also add an inner div layer with background/border/shadow.
       The final restore block clears that layer only during dragover.

   10. Dragover plus sign
       The plus sign is created with:

       section[data-testid="stFileUploaderDropzone"].<drag-class>::before

       It must only appear during dragover.
       It must not appear in normal state.
       It must not appear in hover state.

   11. Processing screen
       Processing screen CSS should be edited separately from upload rectangle CSS.

       Rule:
       - processing screen changes may share colors and typography;
       - processing screen changes must not touch stFileUploader selectors;
       - processing screen changes must not edit the final dragover restore block.

   12. Safe edit protocol
       Before changing upload rectangle CSS:
       - create a backup under back/
       - inspect current DOM in Chrome DevTools
       - verify data-testid selectors
       - verify current dragover class
       - change only the smallest possible block
       - keep normal/hover/dragover logically separated
       - run local app
       - test click upload
       - test drag upload
       - test hover without drag
       - only then commit

   13. Hard rule for future edits
       Do not delete or “clean up” this comment block just because it is long.
       The verbosity is intentional. It prevents repeating the same uploader
       regression.
   ============================================================================= */
...
@import url('https://fonts.googleapis.com/css2?family=Archivo+Black&display=swap');
        ...
        * {
        ...
        @media (max-height: 850px) {
            ...
            .screen-title { font-size: 28px !important; margin-bottom: var(--s4) !important; }
        ...
        @media (max-width: 760px) {
            ...
            .section-title { font-size: 28px !important; margin-top: var(--s5) !important; margin-bottom: var(--s3) !important; }
...
div[data-testid="stFileUploader"] section[data-testid="stFileUploaderDropzone"]::before {
    content: "📎 Drop or upload" !important;
    font-family: "Garet", Inter, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif !important;
    font-size: 28px !important;
    font-weight: 500 !important;
    letter-spacing: 0.01em !important;
}
...
/* =============================================================================
   UPLOAD_DRAGOVER_CLASS_BASED_V1_8_6_LOCKED_DO_NOT_EDIT

   Fix ONLY dragover state for the first upload screen.

   This intentionally does NOT style normal state.
   This intentionally does NOT style hover state.
   It only applies when Streamlit adds the known dragover class:

   section[data-testid="stFileUploaderDropzone"].st-emotion-cache-1n7lb4w

   If Streamlit changes the generated class, replace only:
   st-emotion-cache-1n7lb4w
   after checking DevTools.
   ============================================================================= */


/* 01. Dragover parent rectangle */
...
/* 02. Kill Streamlit native dragover inner layer */
.stApp:has(.upload-screen-active)
div[data-testid="stFileUploader"]
section[data-testid="stFileUploaderDropzone"].st-emotion-cache-1n7lb4w > div,
.stApp:has(.upload-screen-active)
...
/* 03. Hide native Streamlit dragover text */
.stApp:has(.upload-screen-active)
div[data-testid="stFileUploader"]
section[data-testid="stFileUploaderDropzone"].st-emotion-cache-1n7lb4w span,
.stApp:has(.upload-screen-active)
div[data-testid="stFileUploader"]
section[data-testid="stFileUploaderDropzone"].st-emotion-cache-1n7lb4w span * {
    display: none !important;
...
/* 04. Remove possible native after overlay */
.stApp:has(.upload-screen-active)
div[data-testid="stFileUploader"]
section[data-testid="stFileUploaderDropzone"].st-emotion-cache-1n7lb4w::after {
    content: "" !important;
    display: none !important;
    background: transparent !important;
    border: 0 !important;
...
# === COSTERLY_CUSTOM_UPLOAD_BLOCK_V1_9_START ===
def apply_custom_upload_block_css_v1_9():
    import streamlit as st
...
:root {
  --costerly-upload-purple: #3B2E48;
  --costerly-upload-indigo: #8049C6;
  --costerly-upload-white: #FFFFFF;
  --costerly-upload-peach: #FFE1D2;
  --costerly-upload-orange: #FF8A3D;
  --costerly-upload-lilac: #E8D9FF;

  --costerly-upload-width: 600px;
  --costerly-upload-height: 200px;
...
div[data-testid="stFileUploader"] section[data-testid="stFileUploaderDropzone"] {
  ...
  width: 100% !important;
  height: var(--costerly-upload-height) !important;
  min-height: var(--costerly-upload-height) !important;

  padding: 0 !important;
  margin: 0 !important;

  border: 2px solid var(--costerly-upload-purple) !important;
  border-radius: var(--costerly-upload-radius) !important;

  background: var(--costerly-upload-white) !important;
  box-shadow: none !important;
  outline: none !important;
...
div[data-testid="stFileUploader"] section[data-testid="stFileUploaderDropzone"]::before {
  ...
  color: var(--costerly-upload-purple) !important;
...
@media (max-width: 700px) {
  :root {
    --costerly-upload-width: min(92vw, 600px);
    --costerly-upload-height: 180px;
  }
